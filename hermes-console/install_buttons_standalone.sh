#!/usr/bin/env bash
# HERMES 콘솔 복사/붙여넣기 버튼 설치 (완전 자립형 — 외부 다운로드 불필요)
# 안전: ① 백업 먼저 ② 주입 후 콘솔이 안 뜨면 '자동 원상복구'. 콘솔을 죽은 채 두지 않는다.
set -u

# === 1단계: 18091을 서빙하는 node 프로세스 찾기 ===
P=""
...
grep -qF "</body>" "$F" || { echo "결과=이 server.js에 </body> 없음 — 중단(안전)"; exit 1; }

# === 3단계: 버튼 코드 주입 (자립형 — 내장됨) ===
python3 - "$F" <<'PYEOF'
import sys
f = sys.argv[1]
